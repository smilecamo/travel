<template>
  <div>
    <div class="recommend-title">
      <img class="recomment-hot" src="http://img1.qunarzz.com/piao/fusion/1711/89/ebc329f16c55bb02.png" alt="">
      <span>最新热映</span>
    </div>
    <ul class="recomment-ul">
      <li class="item border-bottom" v-for="item of list" :key="item.id">
        <img class="item-img" :src="item.images.large" alt="">
        <div class="item-detail">
          <p class="item-title">{{item.title}}</p>
          <p class="item-comment">类别:{{item.genres[0]}};{{item.genres[1]}};{{item.genres[2]}}</p>
          <div class="item-price-div">
            <span class="item-price">
              <em>{{item.year}}</em>
            </span>
            <span>年</span>
            <span class="item-text">{{item.subtype}}</span>
          </div>
        </div>
      </li>
    </ul>
